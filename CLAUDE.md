...
### 블로그화하는 것은 리뷰가 아니라 논문이다

**사용자가 2026년 9월에 확정한 규칙이다.** `PR{N}-*.html`은 **강의 슬라이드를 정리한 것과
같은 방식으로 논문의 요지를 정리한 학습 노트**다. 리뷰의 결과물이 아니다.

- 슬라이드 노트가 슬라이드를 한 장씩 따라가듯, **PR 노트는 논문의 절을 순서대로 따라간다.**
- **판단은 싣지 않는다.** 강점, 약점, 신규성/임팩트 평가, 종합 판정, 저자에게 보내는 코멘트.
  이건 사용자가 리뷰 폼에 쓰는 것이고 이 페이지의 일이 아니다.
- **논문이 무엇을 말했는가**만 싣는다. 논문이 스스로 붙인 단서("이 예는 과장한다",
  "이 비교는 편향되어 있다")를 함께 옮기면 그 자체가 값어치가 된다.
- **강의와의 연결을 적극적으로 짚는다.** 사용자가 명시적으로 요청한 것이다.
  실제로 L01 34쪽의 두 그래프는 Golden Age 논문의 Figure 2와 3이고,
  L02-3 17~20쪽은 AlexNet 5절과 한 줄씩 대응한다. **슬라이드가 한 줄로 줄인 것의
  원문이 어디인지 알려주는 것**이 PR 노트의 고유한 값어치다.

리뷰 자체에 대해서는 여전히 다음이 유효하다.

1. 사용자가 논문을 읽고 리뷰를 쓴다.
2. **요청하면** 내가 읽고 평가한다. 논문의 실제 내용과 대조해서 사실 오류가 있는지,
   놓친 핵심 기여가 있는지, 약점 지적이 근거 있는지. **리뷰를 고쳐주지 않는다.**
   무엇이 어긋났는지 알려주고 사용자가 고친다.
3. Canvas 점수가 나오면 사용자가 알려준다. **교수의 채점 성향을 추정해서 이 문서에 반영한다.**
...
Paper Reading 노트를 만들 때는 위 [블로그화하는 것은 리뷰가 아니라 논문이다](#블로그화하는-것은-리뷰가-아니라-논문이다)를 따른다.
**사용자가 제출을 마친 뒤에만 만든다.**
